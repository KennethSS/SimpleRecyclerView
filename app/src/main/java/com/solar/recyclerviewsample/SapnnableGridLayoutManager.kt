...
/* XML constructor, see RecyclerView#createLayoutManager *//*
    ...
    override fun scrollVerticallyBy(dy: Int, recycler: Recycler, state: RecyclerView.State): Int {
        ...
        if (dy < 0) { // scrolling content down
            ...
            if (bottom - scrolled < height && lastVisibleRow != 0) { // new row scrolled in
    ...
    override fun smoothScrollToPosition(
        ...
        val scroller: LinearSmoothScroller = object : LinearSmoothScroller(recyclerView.context) {
            override fun computeScrollVectorForPosition(targetPosition: Int): PointF? {
                val rowOffset = getRowTopIndex(targetPosition) - firstVisibleRow
                return PointF(0, (rowOffset * cellHeight).toFloat())
            }
        }
...
/**
    ...
    private fun getRowTopIndex(position: Int): Int {
        return if (position < cells!!.size()) {
            val gridCell = cells!![position]
            gridCell.row
        } else {
            -1
        }
...
/**
    ...
    private fun getRowBottomIndex(position: Int): Int {
        return if (position < cells!!.size()) {
            val gridCell = cells!![position]
            gridCell.row - 1 + gridCell.rowSpan
        } else {
            -1
        }
    }
...
/**
    ...
    private fun layoutRow(
        ...
        if (firstPositionInRow < firstVisibleItemPosition) {
            ...
            firstVisibleRow = getRowTopIndex(firstVisibleItemPosition)
        }
